...
int cfg_write_file(char *path)
{
    ...
    fprintf(cfg_fd, "song_update_url_active = %d\n", cfg.main.song_update_url_active);
    fprintf(cfg_fd, "song_update_url_interval = %d\n", cfg.main.song_update_url_interval);

    if (cfg.main.song_update_url != NULL) {
        fprintf(cfg_fd, "song_update_url = %s\n", cfg.main.song_update_url);
    }
    else {
        fprintf(cfg_fd, "song_update_url =\n");
    }
    ...
    fprintf(cfg_fd, "signal_threshold = %f\n", cfg.main.signal_threshold);
    fprintf(cfg_fd, "silence_threshold = %f\n", cfg.main.silence_threshold);
    ...
    fprintf(cfg_fd, "check_for_buttm = %d\n", cfg.main.check_for_buttm);
    ...
    fprintf(cfg_fd,
            ...
            "silence_detection = %d\n"
            "folder = %s\n\n",
            cfg.rec.bitrate, cfg.rec.codec, cfg.rec.start_rec, cfg.rec.stop_rec, cfg.rec.rec_after_launch, cfg.rec.overwrite_files, cfg.rec.sync_to_hour,
            cfg.rec.split_time, cfg.rec.filename, cfg.rec.signal_threshold, cfg.rec.silence_threshold, cfg.rec.signal_detection, cfg.rec.silence_detection,
            cfg.rec.folder);
    ...
    fprintf(cfg_fd,
            ...
            cfg.gui.default_stream_info, cfg.gui.start_minimized, cfg.gui.disable_gain_slider, cfg.gui.show_listeners, cfg.gui.listeners_update_rate,
            cfg.gui.lang_str, cfg.gui.vu_low_color, cfg.gui.vu_mid_color, cfg.gui.vu_high_color, cfg.gui.vu_mid_range_start, cfg.gui.vu_high_range_start,
            cfg.gui.always_show_vu_tabs, cfg.gui.window_title, cfg.gui.vu_mode);
    ...
    fprintf(cfg_fd,
            ...
            cfg.mp3_codec_stream.vbr_min_bitrate, cfg.mp3_codec_stream.vbr_max_bitrate, cfg.mp3_codec_stream.vbr_force_min_bitrate,
            cfg.mp3_codec_stream.resampling_freq, cfg.mp3_codec_stream.lowpass_freq_active, cfg.mp3_codec_stream.lowpass_freq,
            cfg.mp3_codec_stream.lowpass_width_active, cfg.mp3_codec_stream.lowpass_width, cfg.mp3_codec_stream.highpass_freq_active,
            cfg.mp3_codec_stream.highpass_freq, cfg.mp3_codec_stream.highpass_width_active, cfg.mp3_codec_stream.highpass_width);
    ...
    fprintf(cfg_fd,
            ...
            cfg.mp3_codec_rec.vbr_min_bitrate, cfg.mp3_codec_rec.vbr_max_bitrate, cfg.mp3_codec_rec.vbr_force_min_bitrate, cfg.mp3_codec_rec.resampling_freq,
            cfg.mp3_codec_rec.lowpass_freq_active, cfg.mp3_codec_rec.lowpass_freq, cfg.mp3_codec_rec.lowpass_width_active, cfg.mp3_codec_rec.lowpass_width,
            cfg.mp3_codec_rec.highpass_freq_active, cfg.mp3_codec_rec.highpass_freq, cfg.mp3_codec_rec.highpass_width_active, cfg.mp3_codec_rec.highpass_width);
    ...
    for (i = 0; i < cfg.main.num_of_srv; i++) {
        if (cfg.srv[i]->type != WEBRTC) {
            ...
            fprintf(cfg_fd,
                    ...
                    "type = %d\n"
                    "tls = 0\n",
                    cfg.srv[i]->name, WEBRTC);
        }

        if (cfg.srv[i]->custom_listener_url != NULL) {
            fprintf(cfg_fd, "custom_listener_url = %s\n", cfg.srv[i]->custom_listener_url);
        }
        else {
            fprintf(cfg_fd, "custom_listener_url = \n");
        }

        if (cfg.srv[i]->custom_listener_mount != NULL) {
            fprintf(cfg_fd, "custom_listener_mount = %s\n", cfg.srv[i]->custom_listener_mount);
        }
        else {
            fprintf(cfg_fd, "custom_listener_mount = \n");
        }
        ...
        if (cfg.srv[i]->type == WEBRTC) {
            if (cfg.srv[i]->webrtc_ice != NULL && strlen(cfg.srv[i]->webrtc_ice) > 0) {
                fprintf(cfg_fd, "webrtc_ice = %s\n", cfg.srv[i]->webrtc_ice);
            }
            else {
                fprintf(cfg_fd, "webrtc_ice = \n");
            }

            fprintf(cfg_fd, "webrtc_whip = %s\n", cfg.srv[i]->webrtc_whip);

            if (cfg.srv[i]->webrtc_auth != NULL && strlen(cfg.srv[i]->webrtc_auth) > 0) {
                fprintf(cfg_fd, "webrtc_auth = %s\n\n", cfg.srv[i]->webrtc_auth);
            }
            else {
                fprintf(cfg_fd, "webrtc_auth = \n\n");
            }
        }
        else {
            fprintf(cfg_fd, "\n");
...
int cfg_set_values(char *path)
{
    ...
    if (cfg_parse_file(path) == -1) {
        DEBUG_LOG("Parsing config failed");
        return 1;
    }
    ...
    cfg.rec.overwrite_files = cfg_get_int("record", "overwrite_files", 0);
    ...
    cfg.rec.signal_threshold = cfg_get_float("record", "signal_threshold", 0);
    cfg.rec.silence_threshold = cfg_get_float("record", "silence_threshold", 0);
    ...
    if (cfg.main.num_of_srv > 0) {
        ...
        for (i = 0; srv_ent != NULL; i++) {
            ...
            if ((cfg.srv[i]->addr) == NULL && (cfg.srv[i]->type != WEBRTC)) {
            ...
            if ((cfg.srv[i]->port) == -1 && (cfg.srv[i]->type != WEBRTC)) {
            ...
            if ((cfg.srv[i]->pwd == NULL) && (cfg.srv[i]->type != WEBRTC)) {
            ...
            cfg.srv[i]->webrtc_ice = cfg_get_str(srv_ent, "webrtc_ice", NULL);

            cfg.srv[i]->webrtc_whip = cfg_get_str(srv_ent, "webrtc_whip", NULL);
            if ((cfg.srv[i]->webrtc_whip == NULL) && (cfg.srv[i]->type == WEBRTC)) {
                cfg.srv[i]->webrtc_whip = (char *)malloc(strlen("(none)") + 1);
                strcpy(cfg.srv[i]->webrtc_whip, "(none)");
            }

            cfg.srv[i]->webrtc_auth = cfg_get_str(srv_ent, "webrtc_auth", NULL);
            ...
            cfg.srv[i]->custom_listener_url = cfg_get_str(srv_ent, "custom_listener_url", NULL);
            cfg.srv[i]->custom_listener_mount = cfg_get_str(srv_ent, "custom_listener_mount", NULL);
    ...
    // song update from URL
    cfg.main.song_update_url_active = cfg_get_int("main", "song_update_url_active", 0);
    cfg.main.song_update_url_interval = cfg_get_int("main", "song_update_url_interval", 1);
    cfg.main.song_update_url = cfg_get_str("main", "song_update_url", NULL);
    ...
    cfg.main.signal_threshold = cfg_get_float("main", "signal_threshold", 0);
    cfg.main.silence_threshold = cfg_get_float("main", "silence_threshold", 0);
    ...
    cfg.main.check_for_buttm = cfg_get_int("main", "check_for_buttm", 1);
    ...
    cfg.gui.default_stream_info = cfg_get_int("gui", "default_stream_info", STREAM_TIME);
    ...
    cfg.gui.listeners_update_rate = cfg_get_int("gui", "listeners_update_rate", 10);
    cfg.gui.vu_mode = cfg_get_int("gui", "vu_mode", VU_MODE_SOLID);
    cfg.gui.always_show_vu_tabs = cfg_get_int("gui", "always_show_vu_tabs", 1);

    cfg.gui.vu_low_color = cfg_get_int("gui", "vu_low_color", (int)fl_rgb_color((uchar)0, (uchar)210, (uchar)0));   // Green
    cfg.gui.vu_mid_color = cfg_get_int("gui", "vu_mid_color", (int)fl_rgb_color((uchar)230, (uchar)230, (uchar)0)); // Yellow
    cfg.gui.vu_high_color = cfg_get_int("gui", "vu_high_color", (int)fl_rgb_color((uchar)200, (uchar)0, (uchar)0)); // Red

    cfg.gui.vu_mid_range_start = cfg_get_int("gui", "vu_mid_range_start", -12); // dB
    cfg.gui.vu_high_range_start = cfg_get_int("gui", "vu_high_range_start", -6);

    int lang_id = cfg_get_int("gui", "lang", -1);
    if (lang_id != -1) { // For compatibility with bnbutt configurations <= 0.1.33
        cfg.gui.lang_str = NULL;
        lang_id_to_str(lang_id, &cfg.gui.lang_str, LANG_MAPPING_OLD);
    }
    else {
        cfg.gui.lang_str = cfg_get_str("gui", "lang_str", "system"); // bnbutt > 0.1.33
    ...
    cfg.main.bg_color = cfg_get_int("main", "bg_color", (int)fl_rgb_color(15, 15, 15));      // dark gray
    cfg.main.txt_color = cfg_get_int("main", "txt_color", (int)fl_rgb_color(255, 255, 255)); // white
    ...
    cfg.mp3_codec_stream.vbr_force_min_bitrate = cfg_get_int("mp3_codec_stream", "vbr_force_min_bitrate", 0);
    cfg.mp3_codec_stream.resampling_freq = cfg_get_int("mp3_codec_stream", "resampling_freq", CHOICE_MP3_RESAMPLING_DISABLED);
    cfg.mp3_codec_stream.lowpass_freq_active = cfg_get_int("mp3_codec_stream", "lowpass_freq_active", 0);
    cfg.mp3_codec_stream.lowpass_freq = cfg_get_float("mp3_codec_stream", "lowpass_freq", 0);
    cfg.mp3_codec_stream.lowpass_width_active = cfg_get_int("mp3_codec_stream", "lowpass_width_active", 0);
    cfg.mp3_codec_stream.lowpass_width = cfg_get_float("mp3_codec_stream", "lowpass_width", 0);
    cfg.mp3_codec_stream.highpass_freq_active = cfg_get_int("mp3_codec_stream", "highpass_freq_active", 0);
    cfg.mp3_codec_stream.highpass_freq = cfg_get_float("mp3_codec_stream", "highpass_freq", 0);
    cfg.mp3_codec_stream.highpass_width_active = cfg_get_int("mp3_codec_stream", "highpass_width_active", 0);
    cfg.mp3_codec_stream.highpass_width = cfg_get_float("mp3_codec_stream", "highpass_width", 0);
    ...
    cfg.mp3_codec_rec.vbr_force_min_bitrate = cfg_get_int("mp3_codec_rec", "vbr_force_min_bitrate", 0);
    cfg.mp3_codec_rec.resampling_freq = cfg_get_int("mp3_codec_rec", "resampling_freq", CHOICE_MP3_RESAMPLING_DISABLED);
    cfg.mp3_codec_rec.lowpass_freq_active = cfg_get_int("mp3_codec_rec", "lowpass_freq_active", 0);
    cfg.mp3_codec_rec.lowpass_freq = cfg_get_float("mp3_codec_rec", "lowpass_freq", 0);
    cfg.mp3_codec_rec.lowpass_width_active = cfg_get_int("mp3_codec_rec", "lowpass_width_active", 0);
    cfg.mp3_codec_rec.lowpass_width = cfg_get_float("mp3_codec_rec", "lowpass_width", 0);
    cfg.mp3_codec_rec.highpass_freq_active = cfg_get_int("mp3_codec_rec", "highpass_freq_active", 0);
    cfg.mp3_codec_rec.highpass_freq = cfg_get_float("mp3_codec_rec", "highpass_freq", 0);
    cfg.mp3_codec_rec.highpass_width_active = cfg_get_int("mp3_codec_rec", "highpass_width_active", 0);
    cfg.mp3_codec_rec.highpass_width = cfg_get_float("mp3_codec_rec", "highpass_width", 0);
