...
#ifndef BUILD_CLIENT
    printf(
        "Usage: bnbutt [-h | -v | -s [name] | -u <song name> | -M <streaming signal threshold> | -m <streaming silence threshold> | -O <recording signal threshold> | -o <recording silence threshold> | -SLdrtqn | -c <config_path>] [-A | -U | -x] [-a <addr>] [-p <port>]\n");
#else
    printf(
        "Usage: bnbutt-client [-h | -v | -s [name] | -u <song name> | -M <streaming signal threshold> | -m <streaming silence threshold> | -O <recording signal threshold> | -o <recording silence threshold> | -USdrtqn ] [-a <addr>] [-p <port>]\n");
#endif
    fflush(stdout);
}
...
void load_AAC_lib(void)
{
    ...
    if (dylib == NULL) {
        dylib = dlopen("/Library/Application Support/bnbutt/libfdk-aac.2.dylib", RTLD_LAZY); //  New path since 0.1.34
    }
...
int read_cfg(void)
{
    ...
    if (shift_pressed) {
        int rc = fl_choice(_("The shift key was held down during startup.\n"
                             "Do you want to start bnbutt with a new configuration file?\n"
    ...
    if (cfg_set_values(NULL) != 0) // read config file and initialize config struct
    {
        ...
        printf(_("bnbutt created a default config at\n%s\n"), cfg_path);
        fflush(stdout);
        cfg_set_values(NULL);
    }
...
int main(int argc, char *argv[])
{
    ...
    while ((opt = getopt(argc, argv, ":vhc:AULxs:drtnqu:a:p:SM:m:O:o:")) != -1) {
        ...
        case 'h':
            ...
            printf(_("\nOptions for control mode:\n"
                     ...
                     "-a\tAddress of the bnbutt instance to be controlled (default: 127.0.0.1)\n"
                     "-p\tPort of the bnbutt instance to be controlled (default: 1256)\n"));
            fflush(stdout);
            return 0;
        case '?':
            printf(_("Illegal option -%c.\nType bnbutt -h to get a list of supported options.\n"), optopt);
            print_usage();
            return 1;
        case ':':
    ...
    if (command.cmd != CMD_EMPTY) {
        ...
        case CMD_ERR_CONNECT:
            printf(_("No bnbutt instance running on %s at port %d\n"), server_addr, port);
            fflush(stdout);
            return 1;
            break;
    ...
    if (Fl::get_key(FL_Control_L) || Fl::get_key(FL_Control_R)) {
        fl_alert(_("The control key was held down during startup.\n"
                   "bnbutt will start without opening an audio device.\n"
    ...
    fl_g = new flgui();
    fl_g->window_main->xclass("bnbutt_FLTK");
    ...
    if ((skip_opening_audio_device == 0) && (snd_open_streams() != 0)) {
        ...
        if (snd_open_streams() != 0) {
            ...
            fl_alert(_("bnbutt could not open previously used audio device.\nThe system default audio device will be used.\n"));
        }
    }
    vu_init();
    ...
    if (cfg.main.check_for_update) {
        ...
        if (ret == UPDATE_NEW_VERSION) {
            ...
            if (rc == 2) {
                snprintf(uri, sizeof(uri) - 1, "https://buttm.app");
                fl_open_uri(uri);
            }
        }
    }
    ...
    if (cfg.main.check_for_update && cfg.main.check_for_buttm) {
        int rc;
        char uri[100];
        int ret = update_check_for_BUTTM();
        if (ret == UPDATE_BUTTM_AVAILABLE) {
            char *BUTTM_text = update_get_BUTTM_text();
            rc = fl_choice(("%s"), _("Cancel"), _("Find out more"), NULL, BUTTM_text, NULL);
            if (rc == 1) {
                snprintf(uri, sizeof(uri) - 1, "https://buttm.app");
                fl_open_uri(uri);
            }
            // Do inform user about BUTTM only once in lifetime
            cfg.main.check_for_buttm = 0;
    ...
    fl_g->radio_add_srv_webrtc->tooltip(_("bnbutt was built without WebRTC support"));
#endif
